...
export default function Search({}) {
    ...
    async function handleSearchAction(prevFormState, formData) {
        ...
        const response = await fetch(
            `http://localhost:8000/products/search?category=${search}`,
        );
        const data = await response.json();

        // navigate("/shop", {
        //     state: { filteredProducts: data, category: search },
        // });
        userProgressCtx.hideSearch();
    }

    const [formState, formAction] = useActionState(handleSearchAction, {
        errors: null,
    });
    ...
    return (
        <Modal
            ...
            onClose={
                userProgressCtx.progress === "search" ? handleHideSearch : null
            }
        >
            {/*BACK BUTTON*/}
            <button onClick={handleHideSearch} className={styles.closeBtn}>
